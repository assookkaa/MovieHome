...
<body>
    ...
    include 'viewmov.php';
    $delete = new movieedit;
    $viewmov = new movie();
    $view = $viewmov->viewmovie();
    if (isset($_POST['Delete'])) {
        $movieId = $_POST['movie_id'];
        $confirm = $delete->deleteMovie($movieId);
        if ($confirm) {
            echo 'Movie deleted';
        }
    }
    ...
    ?>
    <a href="addmovie.php">
        <button style="padding:2rem; font-size: 30px; font-weight: bold;">Add a Movie</button>
    </a>
    <div class="movie-list">
        <?php foreach ($view as $movi) { ?>
            <div class="movie">
                ...
                <!-- Add an edit button that links to an edit page -->
                <a href="editmovie.php?movie_id=<?php echo $movi['movie_id']; ?>">
                    <button style="margin: 2px;" type="button">Edit</button>
                </a>
                <form method="POST">
                    <input type="hidden" name="movie_id" value="<?php echo $movi['movie_id']; ?>">
                    <button style="margin: 2px;" type="submit" name="Delete">Delete</button>
                </form>
